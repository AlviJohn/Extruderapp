...
st.sidebar.write('\n\n   This analysis was conducted for 10 YXA Extrusion\n Work Orders shared by Chethan. \n\n   Pushout/Rampup/Stable Time Period for each extrusion was individually identified to understand the parameter variations across these phases of extrusion ')
...
fig.update_layout(title_text= "Line Speed VS " + option + " during Pushout/Rampup/Stable Time Period")
st.plotly_chart(fig,use_container_width=True)

fig = go.Figure()
fig = px.scatter(Raw_data,y='width_rws1_OS', x=option, color='Status')
fig.update_layout(title_text= "Width VS " + option + " during Pushout/Rampup/Stable Time Period")
st.plotly_chart(fig,use_container_width=True)

   
if make_choice[0] != "ALL":
    fig = go.Figure()
    fig = px.line(Raw_data,x='start_time', y=option, color='Status')
    fig.update_layout(title_text= "Extruder Time Vs " + option + " during Pushout/Rampup/Stable Time Period")
    st.plotly_chart(fig,use_container_width=True)
    
    fig = go.Figure()
    fig = px.line(Raw_data,x='start_time', y='PCRQPLX_A_takeaway_speed', color='Status')
    fig.update_layout(title_text= "Extruder Time Vs Takeaway Speed during Pushout/Rampup/Stable Time Period")
    st.plotly_chart(fig,use_container_width=True)
    
else:
    st.write('Extruder Time Series Chart works only on selecting individual work order in the filter')
